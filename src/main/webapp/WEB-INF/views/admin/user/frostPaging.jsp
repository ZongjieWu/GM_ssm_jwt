    ...
    <div class="layui-card">
        <div class="layui-form layui-card-header layuiadmin-card-header-auto">
            <div class="layui-form-item">
                ...
                <%--</div>--%>
                <div class="layui-inline">
                    <a class="layui-btn layui-btn-normal layuiadmin-btn-useradmin" data-type="batchActive"><i class="layui-icon layui-icon-ok"></i><span class="toolFont">批量激活</span></a>
                </div>
                <%--<div class="layui-inline">--%>
                    <%--<a class="layui-btn layui-btn-danger layuiadmin-btn-useradmin" data-type="unBatchPass"><i class="layui-icon layui-icon-close"></i>批量不通过</a>--%>
                <%--</div>--%>
                <div class="toRight">
        ...
        <div class="layui-card-body">
            ...
            <script type="text/html" id="table-useradmin-webuser">
                ...
                <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="pass"><i class="layui-icon layui-icon-ok"></i>激活</a>
            ...
            <script type="text/html" id="authenticationStatusTpl">
                {{#if (d.authenticationStatus==undefined||d.authenticationStatus == 0) { }}
                未认证
                {{# } }}
    ...
    layui.use(['form','layer','table'], function(){
        ...
        table.render({
            ...
            ,cols: [[ //表头
                ...
                ,{field: 'headImg',width:"10%",  align:'center', title: '头像'
                    ,templet:"#imgTpl"
                }
...
//                    ,templet:"#nameDemo"
                ...
                ,{field: 'email' ,width:"13%",align:'center', title: '邮箱'}
                ,{field: 'age' ,width:"5%",align:'center', title: '年龄'}
...
//                }
        ...
        var active = {
            batchActive: function(){
                ...
                layer.confirm("确定通过吗?",function(resdata){
                    $.post('<%=path%>/user/batchCheck',{ids:ids,status:1},
                        function (res) {
                            if (res.code==0){
        ...
        table.on('tool(demo)', function(obj){
            ...
            // 通过
            if (obj.event === 'pass') {
                layer.confirm("确定激活吗?",function(resdata){
                    $.post('<%=path%>/user/update',{id:data.id,status:1},
                        function (res) {
                            if (res.code==0){
                                refreshTable(res.msg);
                            }else {
            ...
            if (obj.event === 'unPass') {
                layer.confirm("确定不通过吗?",function(resdata){
                    $.post('<%=path%>/user/update',{id:data.id,status:2},
                        function (res) {
                            if (res.code==0){
                                refreshTable(res.msg);
                            }else {
