...
//    private static String user_resume_send_template_id="V0y115-Mh8EXwknDBlH98aQa_ie44NuM0n_8QuKFgaQ";
    private static String do_main="d1891w9773.51mypc.cn";
    private static String template_id="F2Px2Y1g94wUaI6s583fYkesJysa9JztaUT3RCdCM-o";
    private static String user_resume_send_template_id="K4f4XO_7Adq07AJeJfTLt96_J1QGasMxT5Zx7P1BFPM";
...
//        userButtonMapList.add(userSubButtonParamMap2);
    ...
    public static String userResumeSendPush(WeChatPublicConfig weChatPublicConfig,Map<String,Object> map){
        System.out.println("weChatPublicConfig.getAccessToken()"+weChatPublicConfig.getAccessToken());
        String result="";
        String url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token="+weChatPublicConfig.getAccessToken();
        try {
            result = HttpClientManager.postUrlData(url,WeChatUtil.userResumeSendPushConfig(map));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return result;
...
//        paramDataMap.put("concatPhone",concatPhone);
    ...
    public static String userResumeSendPushConfig(Map<String,Object> param){
        SimpleDateFormat simpleDateFormat=new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        Map<String,Object> paramMap=new HashMap<>();
        paramMap.put("touser",param.get("openid"));
        paramMap.put("template_id",user_resume_send_template_id);
        paramMap.put("url","http://"+do_main+"/weChatPublic/enterprise/receiveResume.html");
        Map<String,Object> paramDataMap=new HashMap<>();
        Map<String,Object> notifyDate=new HashMap<>();
        notifyDate.put("value",param.get("jobName"));
        notifyDate.put("color","#173177");
        paramDataMap.put("first",notifyDate);
        Map<String,Object> enterpriseName=new HashMap<>();
        enterpriseName.put("value",param.get("userName"));
        enterpriseName.put("color","#173177");
        paramDataMap.put("keyword1",enterpriseName);
        Map<String,Object> jobName=new HashMap<>();
        jobName.put("value",param.get("userEdu"));
        jobName.put("color","#173177");
        paramDataMap.put("keyword2",jobName);
        Map<String,Object> content=new HashMap<>();
        content.put("value",param.get("userWorkAge"));
        content.put("color","#173177");
        paramDataMap.put("keyword3",content);
//        Map<String,Object> concatPhone=new HashMap<>();
//        concatPhone.put("value",param.get("concatPhone"));
//        concatPhone.put("color","#173177");
//        paramDataMap.put("concatPhone",concatPhone);
        Map<String,Object> interviewTime=new HashMap<>();
        interviewTime.put("value",param.get("userPhone"));
        interviewTime.put("color","#173177");
        paramDataMap.put("keyword4",interviewTime);
        paramMap.put("data",paramDataMap);
        Gson gson=new Gson();
        return gson.toJson(paramMap);
    }
