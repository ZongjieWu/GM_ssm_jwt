...
//    }
    ...
    @ApiIgnore
    @PostMapping("paging")
    public Map<String,Object> issuePaging(@RequestParam(defaultValue = "0") Integer page, @RequestParam(defaultValue = "0") Integer limit,String keywords,Long enterpriseId,String workPlace,String age,String sex,String workAge,String edu) {
...
//            }
        ...
        if(age==null||age.equals("")){
            ...
            String[] ageArr=age.split("-");
            paramMap.put("startAge",ageArr[0]);
            paramMap.put("endAge",ageArr[1]);
        }
        if(workAge==null||workAge.equals("")){
        }else{
            String[] workAgeArr=workAge.split("-");
            paramMap.put("startWorkAge",workAgeArr[0]);
            paramMap.put("endWorkAge",workAgeArr[1]);
        }
        if(edu==null||edu.equals("")){
        }else{
...
//        PageInfo<IssueJob> issueJobPageInfo = issueJobService.queryPageListByWhereExample(" id desc",page,limit,example);
        ...
        PageHelper.startPage(page,limit);
        PageInfo<Map<String,Object>> issueJobPageInfo=issueJobService.paging(paramMap);
        Map<String,Object> map=Result.retrunSucessMsgData(issueJobPageInfo.getList());
        map.put("count",issueJobPageInfo.getTotal());
        return map;
    }
