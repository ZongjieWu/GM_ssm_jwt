...
public class ApiUser {
    ...
    public Map<String,Object> login(String phone,String pwd) {
        ...
        if(userMap==null){
            ...
            payload.put("loginId", userMap.get("id"));

            try {
                //86400000是过期时间  24小时
                String jwt = JwtUtils.createJWT("jwt", userMap.get("phone").toString(), 86400000,payload);
                userMap.put("jwt",jwt);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    public Map<String,Object> detail(@RequestParam Long id) {
        ...
        user.setPwd(null);
        try {
            UserResume userResume=userResumeService.findBy("userId",id);
            user.setUserResumeId(userResume.getId());
        }catch (Exception e){
//            e.printStackTrace();
        }
