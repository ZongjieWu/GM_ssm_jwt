...
//    private static String do_main="jxhuadun.cn";
   private static String do_main="d1891w9773.51mypc.cn";
    ...
    @RequestMapping("enterpriseLoginInit")
    @ResponseBody
    public Map<String,Object> enterpriseLoginInit() {
        WeChatPublicConfig weChatPublicConfig = weChatPublicConfigService.findById(1L);
        //回调地址,要跟下面的地址能调通(getWechatGZAccessToken.do)
        String backUrl = "http://"+do_main+"/weixin/weChatPublic/enterpriseAquireCode";
        /**
         *这儿一定要注意！！首尾不能有多的空格（因为直接复制往往会多出空格），其次就是参数的顺序不能变动
         **/
        //AuthUtil.APPID微信公众号的appId
    ...
    @RequestMapping("enterpriseAquireCode")
    public String enterpriseAquireCode(HttpServletRequest request) throws Exception {
        String code = request.getParameter("code");
        return "redirect:http://"+do_main+"/weChatPublic/enterprise/enterprise.html?code="+code;
    }


    @RequestMapping("enterpriseAquireUserInfo")
    @ResponseBody
    public Map<String,Object> enterpriseAquireUserInfo(Long id,String code,HttpServletRequest request) throws Exception {
        ...
        Map<String, Object> data = gson.fromJson(result, Map.class);
        String openid = data.get("openid").toString();
        e.setWechatPublicOpenid(openid);
